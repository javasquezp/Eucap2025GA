...
%bwElo = [20 16];
nCromosome = 300; % Number of possible solutions
...
granularity = 90; % Number of points
dEl = 90/granularity;
dAz = 90/granularity;
...
%binaryMatrix = seedGeneratorConstrained(positionsArray(2, :)./lambda, positionsArray(3, :)./lambda, 6, 0, 0, 0.3, 0.5, nCromosome, maxActive);
%initial_population = [initial_population binaryMatrix];
initialPopulationV = seedGeneratorConstrained(positionsArray(2, :)./lambda, positionsArray(3, :)./lambda, 5, 0, 0, 0.55, 0.3, length(az0), maxActive);
initialPopulationM(k, :) = reshape(initialPopulationV', 1, []);
...
% Define GA options
options = optimoptions('ga', 'InitialPopulationMatrix', initialPopulationM, ...
    'PopulationSize', nCromosome, ...
    'MaxStallGenerations', 1000, ...
    'MaxTime', Inf, ...
    'ConstraintTolerance', 1e-9, ...
    'MaxGenerations', maxGenerations, 'FunctionTolerance', 1e-9, ...
    'FitnessLimit', -Inf, ...            % No early stopping based on fitness
     'PlotFcn', @gaplotbestf, 'MutationFcn', ... 
{@mutationuniform, mutationRate}, 'CrossoverFcn', 'crossoverscattered' , ...
'FitnessLimit', 0.002, 'UseParallel', true);
 ...
 %
    ...
    if ploteron2D
        ...
        s = scatter3(positionsArrayGraph(2,:)./lambda, positionsArrayGraph(3,:)./lambda,...
            sum(abs(wPattern), 2), [], sum(abs(wPattern), 2), 'filled');
        box(axes1,'on');
        hold(axes1,'off');
