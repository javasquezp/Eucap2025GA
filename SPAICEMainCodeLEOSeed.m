...
nElements = 20; %radius of the antenna in lambdas
...
granularity = 90/4; % Number of points
dEl = 90/granularity;
dAz = 90/granularity;
...
initial_population = seedGenerator(positionsArray(2, :)./lambda, positionsArray(3, :)./lambda, 6, 0, 0, 0.3, 1, nCromosome);

for k = 2 : length(az0)

binaryMatrix = seedGenerator(positionsArray(2, :)./lambda, positionsArray(3, :)./lambda, 6, 0, 0, 0.3, 1, nCromosome);
...
% Define GA options
options = optimoptions('ga', 'InitialPopulationMatrix', initial_population, ...
...
'FitnessLimit', 0.02, 'UseParallel', true);
